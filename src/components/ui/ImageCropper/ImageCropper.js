const {
Component, PureComponent, createRef, Fragment,
} = React;
...
class CropImage extends PureComponent {
    ...
    getImagePortion = (imgObject, newWidth, newHeight, startX, startY) => {
        ...
        const {
setIsCropped, getCroppedImg, isFlippedHor, isFlippedVer,
} = this.props;
              ...
              const scaleH = isFlippedHor ? -1 : 1; // Set horizontal scale to -1 if flip horizontal
              const scaleV = isFlippedVer ? -1 : 1; // Set verical scale to -1 if flip vertical
              const posX = isFlippedHor ? imageWidth * -1 : 0; // Set x position to -100% if flip horizontal
              const posY = isFlippedVer ? imageHeight * -1 : 0; // Set y position to -100% if flip vertical
        let scale = 1;
...
class SelectAndCrop extends PureComponent {
    ...
    handleMoveClick = (e) => {
        const newMouseX = e.clientX || e.pageX || (e.touches && e.touches[0].clientX);
        const newMouseY = e.clientY || e.pageY || (e.touches && e.touches[0].clientY);
    ...
    updateWrapperWidth = () => {
        const containerMaxWidth = document.documentElement.clientWidth <= this.img.width
                    ? '100%'
                    : `${this.img.width}px`;

        this.setState({ containerMaxWidth });
...
width, height, x, y,
    ...
    setRatio = (rValue) => {
        const { final: { width } } = this.state;
              const { ratioWidth, ratioHeight, allowUserControls } = this.props;
    ...
    updateWidthHeightRatio = (isRatio, width, height) => {
        const { final: { x, y } } = this.state;
              ...
              const finalWidth = this.state.final.width;
    ...
    setFullWidthToRight = ({ width, final }) => {
        ...
        if (final.x + width <= this.parentWidth) {
            const updatedFinal = { ...this.state.final, width };
...
height, newMouseY, original, final,
    ...
    setFullHeightToBottom = ({ height, final }) => {
        ...
        if (final.y + height <= this.parentHeight) {
            const updatedFinal = { ...this.state.final, height };
    ...
    setRatioToBottomLeft = ({
        ...
        const { widthFromRatio, heightFromRatio } = this.state;
              const newHeight = heightFromRatio * width;
        ...
        if (original.x + (newMouseX - original.mouse_x) >= 0) {
            ...
            * set "width to coming width", "height to ratio height" and "x to initial x + (current mouse x - initial mouse x) */
            if (
                ...
                const newX = original.x + (newMouseX - original.mouse_x);
                      const finalHeight = heightFromRatio * width;
                      const updatedFinal = {
                          ...this.state.final,
                          width,
                          height: finalHeight,
                ...
                if (newHeight + final.y >= this.parentHeight) {
                    ...
                    * set "width to (parent div height - current y) * ratio", "height to parent div height - current y" and "x to (old x + old width) - new width" */
                    const oldOccupiedSpaceX = final.x + final.width;
                          ...
                          const finalHeight = this.parentHeight - final.y;
                          const updatedFinal = {
                              ...this.state.final,
                              width: newWidth,
                              height: finalHeight,
            ...
            if (newHeight + final.y <= this.parentHeight) {
                ...
                const newWidth = final.width + final.x;
                      const finalHeight = heightFromRatio * newWidth;
                      const updatedFinal = {
                          ...this.state.final,
                          width: newWidth,
                          height: finalHeight,
    ...
    setRatioToTopRight = ({
        ...
        const { widthFromRatio, heightFromRatio } = this.state;
              const newWidth = widthFromRatio * height;
        // if inside div on y
        if (original.y + (newMouseY - original.mouse_y) >= 0) {
            if (newWidth + final.x <= this.parentWidth && height > minimum_size) {
                ...
                * set "width to parent div width - current x", "height to ratio height based on new width" and "y to (old height + old y) - new height" */
                if (height > minimum_size) {
                    const oldOccupiedSpace = final.height + final.y;
                          const newHeight = heightFromRatio * (this.parentWidth - final.x);
                          const newY = oldOccupiedSpace - newHeight;
                          const finalWidth = this.parentWidth - final.x;
                          const updatedFinal = {
                              ...this.state.final,
                              width: finalWidth,
                              height: newHeight,
        ...
        } else if (newWidth + final.x <= this.parentWidth) {
                ...
                * set "width to width based on new height", "height to current height + current y" and "y to 0" */
                const finalWidth = widthFromRatio * (final.height + final.y);
                      const finalHeight = final.height + final.y;
                      const updatedFinal = {
                          ...this.state.final,
                          width: finalWidth,
                          height: finalHeight,
        ...
        if (original.y + (newMouseY - original.mouse_y) >= 0) {
            ...
            if (
                ...
                const newHeight = heightFromRatio * (final.width + final.x);
                      const oldOccupiedSpaceY = final.y + final.height;
                      const newY = oldOccupiedSpaceY - newHeight;
                if (
                    newY >= 0
                    && newHeight + final.y <= this.parentHeight
                    && height > minimum_size
            ...
            // opposite
            const newX = oldOccupiedSpaceX - widthFromRatio * (final.height + final.y);
    ...
    pushResizableToLeft = () => {
        const {
 isLeftEnabled, isRightEnabled, final: {
width, height, x, y,
},
} = this.state;
        if (isLeftEnabled) {
            this.setState((prevState) => ({
                ...
                isLeftEnabled: prevState.final.x - 1 !== 0,
            }));
        }
    ...
    pushResizableToRight = () => {
        const {
 isRightEnabled, isLeftEnabled, final: {
width, height, x, y,
},
} = this.state;
    ...
    pushResizableToTop = () => {
        const {
 isTopEnabled, isBottomEnabled, final: {
width, height, x, y,
},
} = this.state;
        if (isTopEnabled) {
            this.setState((prevState) => ({
                ...
                isTopEnabled: prevState.final.y - 1 !== 0,
            }));
        }
    ...
    pushResizableToBottom = () => {
        const {
 isBottomEnabled, isTopEnabled, final: {
width, height, x, y,
},
} = this.state;
...
width, height, x, y,
              ...
              const { image, allowUserControls } = this.props;
